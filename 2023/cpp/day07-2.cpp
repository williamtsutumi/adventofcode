...
bool cmp(pair<string, int> a, pair<string, int> b){
    ...
    string strB = b.first;
    int size = strA.size();
    for (int i=0; i<size; i++){ if (strA[i] != 'J') va[strA[i] - '0']++; }
    for (int i=0; i<size; i++){ if (strB[i] != 'J') vb[strB[i] - '0']++; }

    sort(va.begin(), va.end(), [](int a, int b) -> bool { return a > b; });
    sort(vb.begin(), vb.end(), [](int a, int b) -> bool { return a > b; });

    for (int i=0; i<size; i++) { if (strA[i] == 'J') va[0]++; }
    for (int i=0; i<size; i++) { if (strB[i] == 'J') vb[0]++; }
